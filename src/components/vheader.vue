<template lang="html">
    <header>
      <div class="bg"></div>
        <nav class="header-bg">
          <ul>
            <li><router-link to="/">信息主页</router-link></li>
            <li><router-link to="/query">职位查询</router-link></li>
            <li><router-link to="/about">关于本站</router-link></li>
            <li>
              <router-link v-if="!Login" to='/login'>用户登录</router-link>
              <router-link v-else class="" to='/userinfo'>用户信息</router-link>
            </li>
          </ul>
        </nav>
        <!-- 导航 结束-->
...
<style lang="css" scoped>
  ...
  .bg{
    ...
    background:  url('../assets/image/bgm.jpg') no-repeat center /cover;
